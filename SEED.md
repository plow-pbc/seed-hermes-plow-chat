...
- If Plow emits `chat_active` while the adapter is connected, or if the adapter first connects to an already-active chat, the adapter SHOULD send exactly one setup-success welcome message through the normal Plow message endpoint.
